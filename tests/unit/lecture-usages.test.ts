...
describe("gesteElargir", () => {
  it("vers 7 jours, filtres gardés ; aucun geste déjà sur 7 jours", () => {
    expect(gesteElargir("/paths", requete("app=a&period=24h&device=tablet"))).toEqual({
      libelle: "Élargir à 7 jours",
      href: "/paths?app=a&device=tablet&period=7d",
    });
    expect(gesteElargir("/paths", requete("app=a&period=7d"))).toBeUndefined();
  });
  it("une plage personnalisée est REMPLACÉE, jamais combinée avec period (range_conflict)", () => {
    const geste = gesteElargir("/forms", requete("app=a&from=2026-09-22T10:00:00Z&to=2026-09-22T12:00:00Z"));
