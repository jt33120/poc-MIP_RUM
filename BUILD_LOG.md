...
## v0.3 — Sprint nuit 2 (11/06/2026, « zéro limite »)

- **Méthode** : même protocole que v0.2 (contrat SQL verrouillé d'abord — migration-v03.sql —, 5 agents parallèles sur fichiers disjoints, dépendances épinglées par l'orchestrateur AVANT le build pour éviter les conflits de package.json, intégration/CI/déploiement/recette orchestrés). B4 (ClickHouse) mort une fois sur erreur d'API → relancé seul via resume de workflow (les 4 autres en cache).
- **Ce qui a marché** (valeurs réelles) :
  - Replay : cœur 24,7 KB gzip (+0,9), bundle replay séparé 56,7 KB lazy-loadé ; masquage prouvé (saisie absente du flux) ; E2E complet démo→chunks gzip bytea→player console authentifiée.
  - Alerting sortant : POST webhook réellement reçu (payload Slack-compatible), alert_delivery tracé ; geo tz→pays : 'FR' constaté EN PROD sur une session du vrai site ; rate limit durable : 601e requête refusée, compteur partagé entre 2 connexions.
  - RBAC : 11/11 (dont anti-énumération par hash factice et anti-lockout) ; piège notable : le fallback no-JS des Server Actions Next 15 poste en multipart avec un champ $ACTION_ID — un POST urlencoded renvoie 200 SANS exécuter l'action.
  - ClickHouse : quantileExactInclusive == percentile_cont → Δ=0 exact sur 4 routes × 168 buckets ; CH ×2,2 (séries) / ×12 (count distinct) / disque ×15 ; PG garde l'avantage sur la requête point (~5 ms d'aller-retour HTTP CH incompressible). Piège : healthcheck alpine sur localhost → ::1 alors que CH écoute IPv4.
  - Health score vérifié à la main (SQL) : 72 « Dégradé » global, 35 « Critique » gip-plateforme — le LCP 4 s de /login pèse.
  - Intégration : 91 unitaires + 6 E2E verts, CI verte. Bug d'intégration trouvé par l'orchestrateur : /mip-rum.js déployé était le cœur v0.2 (copie antérieure au rebuild) — détecté par comparaison de taille, resynchronisé.
