...
- **Assistant IA d'intégration** (env-gated `ANTHROPIC_API_KEY`, admin only, rate-limité) : génère le middleware pour une stack non couverte ou diagnostique « rien n'arrive » — system prompt nourri du protocole et des pièges vécus (pydantic-settings, proxy, CSP, head). Sans clé : encart désactivé, le wizard déterministe reste le chemin nominal.
