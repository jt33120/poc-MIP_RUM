...
### API publique v1 (LOT C, option B — console)
- **API REST stable `/api/v1`** exposant les agrégats RUM pour le **front Angular de MIP** (option B du cadrage `docs/CADRAGE_LOT_C.md` : découplage, console livrable seule, **aucune réécriture**). 11 endpoints lecture seule réutilisant la couche data existante (`lib/queries*.ts`, `lib/health.ts`) — **zéro SQL dupliqué** : `apps`, `overview` (health + vitals + stats avec période précédente), `vitals` (+ séries), `pages`, `errors` (+ `{fingerprint}`), `sessions` (+ `{id}`), `tracing`, `correlation`, `health-grid`, et `/api/v1` (découverte).
- **Auth machine** : `Authorization: Bearer <token>` comparé à `CONSOLE_API_TOKENS` (comparaison temps constant, rotation multi-jetons), **ou** cookie de session JWT (RBAC `viewer` scopé préservé). **CORS** par liste blanche (`CONSOLE_API_ALLOWED_ORIGINS`), préflight `OPTIONS` géré. Le middleware laisse passer `/api/v1` (auth dans le handler → 401 JSON propre au lieu d'une redirection `/login`).
- Enveloppe stable `{ meta, data }`, scoping RBAC sur `app`, erreurs typées (`ApiHttpError` → 401/404/500). Helpers purs isolés (`lib/api/{auth,cors,params}.ts`). Contrat documenté dans **`docs/API_CONSOLE.md`** (DTO, env, exemple client Angular). +16 tests unitaires (auth/CORS/scoping). Suite : **214 unitaires** verts.
...
- Docs vente : `docs/OFFRE.md` ([hors dépôt](docs/DOCUMENTS-HORS-DEPOT.md) — comparatif marché honnête — le différenciateur est le créneau OTel/souverain/corrélation, pas la parité fonctionnelle ; pricing 3 paliers indicatif à valider MIP) et `docs/DEMO_SCRIPT.md` (démo 10 min, plan B hors-ligne, objections/réponses).
