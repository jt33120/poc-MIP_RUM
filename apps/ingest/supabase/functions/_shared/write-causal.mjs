 ...
 * puis code v69 / DB v66 à v68 (enveloppe d'erreur P5.1).
 *
...
export function createSchemaCompatibleWriter(supabase, { retry, onRetry }) {
  ...
  const columns = {
    ...
    rum_error: ["span_id", "session_id", "app_id", "route", "kind", "message", "error_type", "stack", "source", "lineno", "colno", "release", "fingerprint", "occurrences", "action_id", ...errorEnvelopeColumns, "ts"],
  ...
  return {
    ...
    async writeCausalChildren({ errors, resources, breadcrumbs, spans, eventIndex }) {
      await ins("rum_error", errorRows(errors), { stripOnOldSchema: [errorEnvelopeColumns, ["action_id"]] });
