...
// tables sont déjà lisibles par console_ro (v16).
//
...
export async function internalHealth(): Promise<HealthSnapshot> {
  ...
  // Une instruction faite de sous-requêtes scalaires rend toujours une ligne :
  // son absence est une anomalie, pas un système au repos.
  if (!r) throw new Error("santé interne : instantané vide");
  return r;
}
