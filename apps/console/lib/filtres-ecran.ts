...
//
// CE MODULE NE LIT PAS LA SESSION (piste C) : il reçoit le principal. La page le
// lui donne par `pageFilters` (`lib/page-filters.ts`, qui lit la session), le
// chargeur d'un écran (`lib/chargeurs/`) directement — dans la console comme dans
// console-api, qui embarque ce module et n'a pas de cookie à lire.
//
...
/** Les filtres d'un écran pour un principal donné (`null` : aucune session, donc aucune app). */
export async function analyserFiltres(
  user: ScopePrincipal | null,
  ...
  // Plus de refus « une application à la fois » (F40 → F53) : toutes les lectures des
  // écrans branchés ici lient les apps EFFECTIVES du principal (`sqlContext`, R-A).
