...
const RAW_GRID = `
  ...
  where m.ts > now() - interval '14 days' and ($1::text is null or m.app_id=$1) and ($2::text is null or s.device_type=$2)
...
const ROLLUP_GRID = `
  ...
  where hour > now() - interval '14 days' and ($1::text is null or app_id=$1) and ($2::text is null or device_type=$2)
...
const RAW_TRAFFIC = `
  ...
  left join (select date_trunc('day', p.started_at) d, count(*)::int n from rum_pageview p left join rum_session s using (session_id)
             where p.started_at > now() - interval '14 days' and ($1::text is null or p.app_id=$1) and ($2::text is null or s.device_type=$2) group by 1) pv on pv.d=gs.day
  left join (select date_trunc('day', e.ts) d, count(*)::int n from rum_error e left join rum_session s using (session_id)
             where e.ts > now() - interval '14 days' and ($1::text is null or e.app_id=$1) and ($2::text is null or s.device_type=$2) group by 1) er on er.d=gs.day
  order by 1`;
const ROLLUP_TRAFFIC = `
  select gs.day::date as day, coalesce(pv.n,0)::int as pageviews, coalesce(er.n,0)::int as errors
  from generate_series(date_trunc('day', now()) - interval '13 days', date_trunc('day', now()), interval '1 day') gs(day)
  left join (select date_trunc('day', hour) d, sum(pageviews)::int n from rum_rollup_hourly
             where hour > now() - interval '14 days' and ($1::text is null or app_id=$1) and ($2::text is null or device_type=$2) group by 1) pv on pv.d=gs.day
  left join (select date_trunc('day', hour) d, sum(errors)::int n from rum_rollup_hourly
             where hour > now() - interval '14 days' and ($1::text is null or app_id=$1) and ($2::text is null or device_type=$2) group by 1) er on er.d=gs.day
  order by 1`;
