...
export function createChWriter({ url = DEFAULT_URL, database = DEFAULT_DB, batchSize = 10_000 } = {}) {
  ...
  async function raw(query, body) {
    const params = new URLSearchParams({ database, query });
  ...
  return {
    /** Exécute une requête (DDL/SELECT). SELECT : passer FORMAT dans le SQL. */
    exec: (sql) => raw(sql),

    /** SELECT → tableau d'objets (FORMAT JSON ajouté automatiquement). */
    async query(sql) {
      const text = await raw(`${sql.trim().replace(/;\s*$/, "")} FORMAT JSON`);
