// `apps/console/lib/db.ts` DANS LE SERVICE `api` : le pool du service, nommé
// `mip-api`, créé par le kit et INJECTÉ par `server.mjs`.
//
...
function obtenir() {
  if (!courant) throw new Error("api : pool non branché (server.mjs doit appeler brancherPool)");
  return courant;
}
