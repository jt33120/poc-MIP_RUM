...
§ 3), `A10`, `D6` et `E2` (commandes rejouées, outils recomptés le 23/09). Après le relevé, la revue de fin de vague 7 a réécrit la limite de `E3`, verdict inchangé : elle refusait toute écriture au viewer, que le code de `8a5f3d1` autorisait déjà sur ses propres tableaux de bord et vues (preuves nommées dans la ligne).
