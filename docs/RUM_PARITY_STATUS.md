...
Relevé le **18 septembre 2026**, sur `master` à **`ed33e27`** (fusion de la PR #209).
...
Toutes les commandes ci-dessous ont été rejouées le 18/09/2026 dans un worktree propre de `ed33e27`,
...
un relevé local qui l'oublie mesure 2 302 tests au lieu de 2 312 et croit avoir un défaut.

**Déploiements constatés le 18/09/2026** (API Railway et API Vercel, pas un journal) :

| Cible | SHA | État | Domaine public |
|---|---|---|---|
| Console Vercel `mip-rum-console` | `ed33e27` | READY, cible `production` | oui |
| Railway `ingest` | `a55d2bd` | SUCCESS, `node services/ingest/server.mjs`, healthcheck `/health` | **aucun** (ni domaine Railway, ni domaine personnalisé) |
| Railway `scheduler` | `a55d2bd` | SUCCESS | aucun |
| Railway `mcp` | `94a6a0c` | SUCCESS | `mcp-production-201c.up.railway.app` |

`a55d2bd` et `94a6a0c` sont en retard sur `ed33e27` d'un seul commit qui ne touche qu'un fichier de
test (`git diff --stat a55d2bd ed33e27` → `tests/integration/analytics-rollups-sql.test.ts`). Ce
retard n'a donc aucune conséquence fonctionnelle.
...
1. **Tout ce qui n'existe que dans l'image Railway ne s'exécute pas sur le trafic réel.** C'est le cas
   de la base GeoIP de P8.7 (§ 5, `D14`).
...
Répartition des verdicts : `deploye_non_eprouve` **32** · `livre_non_deploye` **5** ·
`bloque_acces_externe` **4** · `livre_avec_defaut_connu` **3** · `en_revue` **2** ·
`non_commence` **2** · `non_retenu` **1**.

**Zéro capacité éprouvée sur donnée réelle.** Les 32 lignes `deploye_non_eprouve` sont le meilleur
...
| D12 | Créer un ticket chez un fournisseur depuis une issue | `en_revue` | **PR #211 ouverte, non fusionnée**, CI verte (6 contrôles) ; migration **v84 non appliquée** ; `apps/ingest/lib/integrations/tickets/{adapter,dispatcher}.mjs`, tables `ticket_integration` / `ticket_outbox` / `ticket_webhook_event`. **Un vrai ticket a été créé** dans un dépôt bac à sable — c'est la seule ligne de P5–P8 qui ait touché un système externe réel | **Rien ne tourne en production** : connecteur non déployé, `TICKET_INTEGRATIONS` non posé, interface d'administration cachée tant qu'aucun fournisseur n'est branché **et testé**. Un seul fournisseur (GitHub Issues), présenté dans le produit comme une étape vers l'ITSM de MIP — « ServiceNow, sous réserve de confirmation », et personne n'a confirmé. Aucune écriture MIP → fournisseur après la création : résoudre une issue ne ferme pas le ticket. Le lien manuel de P5.6 continue de fonctionner **sans** connecteur, et un test le prouve. |
| D13 | Recevoir les changements d'état du fournisseur (webhook) | `bloque_acces_externe` | signature HMAC-SHA256 sur corps brut, comparaison à temps constant, identifiant de livraison unique en base ; tests contre un double fidèle (corps modifié, secret tiers, signature tronquée, non hexadécimale, absente) — PR #211 | **Le webhook n'a jamais reçu de livraison d'un vrai fournisseur** : il n'existe pas d'URL publique pour cette branche. Aucune synchronisation de statut n'a été observée en vrai. C'est le premier écart à combler si le connecteur est retenu. |
| D14 | Préciser le pays par une base IP→pays | `en_revue` | **PR #210 ouverte, non fusionnée**, CI verte ; migration **v85 non appliquée** ; `apps/ingest/lib/geoip-db.mjs`, `_shared/geoip.mjs`, `infra/docker/Dockerfile.backend` ; `tests/unit/geoip-db.test.ts`, `tests/integration/geoip-v85-sql.test.ts` | **Le GeoIP ne résoudra aucun pays sur le trafic actuel.** La base est embarquée dans l'**image Railway**, et le service `ingest` n'a pas de domaine public (§ 3) ; la route Vercel, qui reçoit tout le trafic, **n'appelle délibérément pas** la résolution locale — elle ne pose que la provenance `cdn`. La base elle-même n'est pas versionnée et doit être déposée. Pays seulement : ni ville, ni région, ni coordonnées ; la base « City Lite » est refusée par le chargeur. **Aucun enrichissement rétrospectif n'est possible** : l'adresse des visites passées n'a jamais été stockée, et ce lot ne commence pas à la stocker. |
...
| E2 | Lire par MCP | `deploye_non_eprouve` | `apps/mcp/lib/catalogue.mjs` — **16 outils** comptés le 18/09 (`mip_rum_list_apps` … `mip_rum_mobile_summary`) ; job CI `mcp-smoke` qui compare ce nombre ; service Railway `mcp` sur `94a6a0c`, domaine public | Read-only par construction. Le catalogue n'expose **pas encore les dimensions** de P6 : les outils P4/P5 restent sur app, période et appareil. |
...
Avec `D14` fusionné et déployé, ce serait une base IP→pays : elle situerait une **adresse**, souvent
celle d'un opérateur, d'un relais d'entreprise ou d'un VPN. Ce ne serait toujours pas une personne.
Et en l'état, elle ne résoudrait rien (§ 3).
...
**La cause est commune aux trois : une PR fusionnée avant que la CI du *commit de fusion* ait rendu
son verdict.** Une branche verte ne dit rien du résultat de sa fusion. Trois parades sont déjà
écrites dans le code, et elles suffisent si on s'y tient :
...
Mesuré le 18/09 sur `ed33e27`, avec les cinq bases dédiées de la CI : **24 fichiers verts et
2 ignorés — soit 26 au total, pas 24** ; **349 tests verts, 16 ignorés, 365 au total**.
`ls tests/integration/` en compte bien 26. L'écart de tests s'explique : la fenêtre v65 a été fournie
ici et pas là. L'écart de **fichiers**, non : le journal sous-compte l'inventaire de deux fichiers, et
identifie mal les deux suites ignorées — ce sont les **deux bancs** (`explorer-bench-p66`,
`rum-mobile-bench-p75`), tous deux gouvernés par `BENCH_DATABASE_URL`.
...
### 8.2 Deux suites ne tournent jamais en CI, et ce n'est dit nulle part

Comparaison des variables consommées par les tests et de celles fournies par `ci.yml` :

```
consommées : SQL_TEST_DATABASE_URL, SQL_TEST_V65…, SQL_TEST_V68…,
             SQL_TEST_PRE_V71…, SQL_TEST_PRE_V81…, SQL_TEST_PRE_V83…, BENCH_DATABASE_URL
fournies   : SQL_TEST_DATABASE_URL, SQL_TEST_V65…, SQL_TEST_V68…,
             SQL_TEST_PRE_V71…, SQL_TEST_PRE_V81…
```

Deux manquent, et leurs suites sont `describe.skip`-ées **en silence** :

- **`SQL_TEST_PRE_V83_DATABASE_URL`** — la fenêtre de déploiement « code publié **avant**
  migration-v83 » de P8.2. Mesuré : `tests/integration/backfill-idempotency-sql.test.ts` rend
  **49 verts + 3 ignorés** sans la variable, et **52 verts** avec une base au schéma v82. Les trois
  tests **passent** ; simplement, la CI ne les a jamais joués. La CI verte de la PR #208 ne couvre
  donc pas la fenêtre de déploiement que son journal met en avant.
...
- **L'état réel de la base de production** : migrations appliquées, nombre de lignes, date de la
  dernière ingestion, périmètre chiffré de `D9`. `DATABASE_URL` était interdite pour ce travail et n'a
  pas été employée. Tout ce qui touche la production dans ce document est **attribué** à sa source
  (journal de livraison, ou décision du 18/09/2026) et non recontrôlé.
- **Le comportement de la façade Railway sur `X-Real-IP`** (`D14`) : il n'existe aucun déploiement
  exposé sur lequel le sonder.
...
1. **Aucune recette sur vraie application** (§ 6.2). Aucune capacité de ce document n'a franchi la
   colonne « éprouvé sur donnée réelle ».
...
4. **Deux capacités ne sont pas sur `master`** : `D12` et `D14` vivent dans les PR #211 et #210,
   ouvertes et non fusionnées ; leurs migrations v84 et v85 ne sont pas appliquées.
5. **Deux défauts connus, non corrigés** : `tsc --noEmit` du SDK web en échec sans que la CI le joue
   (§ 8.4), et `pnpm -r build` inopérant depuis un dépôt propre (§ 8.3).
...
referme pas non plus les quatre autres.
...
3. **Fusionner ou fermer #210 et #211.** Deux PR vertes qui vieillissent sur une base qui bouge sont
   le décor exact des trois incidents du § 7.
4. **Mettre `tsc --noEmit` et les deux suites orphelines dans la CI** (§ 8.2, § 8.4), puis corriger
   `replay.ts:242` et l'ordre de build de `apps/extension`. Quatre corrections courtes, qui rendent
   vraie la phrase « la CI est verte ».
