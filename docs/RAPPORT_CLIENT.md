...
(**19,4 KB gzip**) capte les Core Web Vitals réels (LCP/INP/CLS/FCP/TTFB), les erreurs JS
...
- **Corrélation lisible** : pour chaque route, robot vs réel côte à côte + écart % ;
  ...
  comme avec le seed.
- **Chiffres mesurés** : bundle 19,4 KB gzip ; p75 console = recalcul manuel (vérifié) ;
...
- **Bundle > 10 KB** : la cible « <10 KB » du rapport v1 est irréaliste pour un SDK
  OTel-natif ; 19,4 KB gzip est le coût du « vrai OTLP sur le fil » (full
