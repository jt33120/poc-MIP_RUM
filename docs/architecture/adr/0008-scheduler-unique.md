...
4. **Le rejeu manuel prend le même bail** (`services/scheduler/run-once.mjs`) ; les routes `/api/cron/*` répondent 410.
5. **La livraison sort du scheduler** (P5) : il décide, le notifier livre (`SCHEDULER_DELIVERY=off`).
