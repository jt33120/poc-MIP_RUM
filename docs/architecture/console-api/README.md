...
- **dans la console** : la page appelle `chargerEcran(ECRANS.overview, chargerOverview, sp)` (`lib/ecran.ts`) — l'opération du contrat et le chargeur qui la sert. Servie par la console, la page exécute le chargeur avec le principal de la session ; servie par `console-api` (la bascule, section plus bas), elle appelle l'opération ;
...
1. **La page lit déjà la forme du fil.** `chargerEcran` passe la sortie du chargeur par JSON (`versLeFil`), exactement ce que fait le service : la page est typée `Fil<…>` (une date y est une chaîne ISO, un `Set` n'y passe pas), et le compilateur refuse l'écran qui l'oublierait. La bascule a changé `chargerEcran` : il appelle l'opération, ou le chargeur, sans qu'aucune page change de type. Chaque page nomme depuis l'opération de son écran ; à la décommission (C12), elle cessera d'importer son chargeur (un `import type` gardera le type) — ce que les server actions font déjà pour leurs commandes (C6) : tant que la page importe le chargeur, elle reste dans le cliquet.
...
Une écriture = une **commande** dans `apps/console/lib/commandes/<domaine>.ts`, enregistrée dans `lib/commandes/index.ts` sous la clé de son opération (`COMMANDES` du contrat — le type refuse une opération sans commande, ou l'inverse). C'est l'exact pendant d'un chargeur : le même code tourne dans la console (`executerCommande`, `lib/commande.ts`) et dans `console-api`, qui l'embarque (`services/console-api/commandes.mjs`) et la sert sous son opération.

1. **La server action appelle la commande PAR SA CLÉ** — `executerCommande("creerObjectif", { app, corps })` — et n'importe aucune commande. Elle lit son formulaire, puis tire de la DÉCISION rendue sa redirection ou sa revalidation. À la bascule, seul `executerCommande` a changé (il appelle l'opération, ou la commande) ; à la décommission (C12), le chemin local disparaît et les fichiers d'actions quittent le cliquet sans changer d'une ligne, et avec eux les composants qui les importent pour leurs formulaires.
...
Ce qu'une page tient de sa requête reste à la page : l'hôte (URL du SDK et de l'ingestion d'un snippet).
...
**C9c — les secrets à usage unique** (mot de passe créé ou réinitialisé, clé d'une application créée ou tournée, jeton de lecture, site ajouté depuis `/select`). La commande rend le secret dans sa décision ; la server action le rend à SON formulaire (`useActionState`, `components/secret/SecretUnique.tsx`), qui le remet à l'écran. L'écran le retire en le lisant :
- sur la même page (un compte, un jeton, une clé tournée), le bandeau l'affiche aussitôt ;
- quand l'affichage est sur une autre page (la fiche de l'application qu'on vient de créer, l'étape 2 de `/select/new`), le formulaire navigue côté client et la page d'arrivée le lit. À l'étape 2, le bookmarklet et la commande de l'agent le portent à la place de leur repère `COLLE_ICI_LA_CLE_API`.
...
**C11b — le hook des tickets, les relais purs, les releases :**
...
1. `console-api` branché (C0b) ;
2. une session de `console-api` (jeton ES256). Une session HS256, signée par la console, n'existe pas pour le service : elle reste servie par la console jusqu'à son expiration, ou jusqu'au retrait d'`AUTH_SECRET` ;
3. le jeton se vérifie (clé publique, sans appel) ;
4. disjoncteur fermé (5 échecs de transport en 30 s → 60 s de console seule, par famille et par instance) ;
5. le tirage : `platform_flag.console_api_ecrans_pct` (écrans et coquille), puis `console_api_commandes_pct` (écritures), cache 30 s, défauts `CONSOLE_API_ECRANS_PCT` / `CONSOLE_API_COMMANDES_PCT`, 0. **Stable par session** : le seau est l'empreinte de l'identifiant de session, 10 % veut dire un dixième des sessions, toujours les mêmes.

**Une lecture** (`lib/ecran.ts`) : la page nomme l'opération ET le chargeur (`chargerEcran(ECRANS.overview, chargerOverview, sp)`) ; `tests/unit/ecrans-operations.test.ts` vérifie que le service sert cette opération avec ce chargeur-là (`services/console-api/ecrans.mjs`), pour chaque appel. Un refus de filtre du service est relevé tel que le chargeur l'aurait levé. Un échec de transport est compté et la console sert. Un refus AVANT le chargeur (session, rôle, périmètre, paramètres) : la console sert aussi — son chargeur rend la décision que l'écran sait afficher — et le journal dit l'écart, à corriger avant le mode strict.

**Une écriture** (`lib/commande.ts`) : l'opération, `?app=` pour une commande de portée `app`, le chemin, le corps. Les refus du service sont ceux de la commande (la server action ne sait pas qui a répondu). **Une écriture ne se rejoue pas** : seul un échec où rien n'est parti (non branché, hôte non vérifié) laisse la console écrire ; tout autre échec a une issue inconnue — l'action lève, avec la référence de la requête.

**La coquille** (`lib/coquille-ecran.ts`, `console.shell`) : même aiguillage que les écrans. En mode strict, sans le service, elle s'affiche partielle (F02) plutôt que de tomber.

**Les routes** qui lisent par un chargeur — `/api/releases`, l'export CSV d'un tableau, le rejeu (portée `all` : le lecteur ne connaît que la session) — passent par `lireEcran` et font du refus du service un statut HTTP.

**Le mode strict** (`CONSOLE_API_STRICT=1`, après le rodage à 100 % sans écart au journal, et après le retrait d'`AUTH_SECRET`) : toute session servie par le service, sans tirage ni disjoncteur, et **aucun échec ne retombe sur la base**. Une page suit alors la porte du middleware (`/login`, l'accueil, `/select`) ; toute autre panne est une erreur d'écran avec la référence. C'est la répétition générale de C12 : la console tourne comme si elle n'avait plus de base.
