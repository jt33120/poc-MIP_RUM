...
> Ce qui reste à faire par une personne : créer le compte développeur, payer les
...
> Au 26/09/2026, aucun compte n'est ouvert (`apps/console/lib/specs.ts`, ligne
> « Compte développeur Chrome »).
...
| **Résumé** (132 car. max) | Mesure la performance réelle (Core Web Vitals) et les erreurs des sites autorisés. Données pseudonymes, hébergées en UE. |
...
> MIP RUM est un capteur de Real User Monitoring (RUM). Il mesure la
...
> • Respect de la vie privée : aucune frappe clavier ni contenu de formulaire, aucune
>   adresse IP stockée ; le visiteur n'est connu que par un identifiant tiré au hasard,
>   qu'il efface en vidant le stockage du site. Hébergement en Union européenne,
...
- **Justification de chaque permission** (à coller dans les champs dédiés) :
  ...
  | `storage` | Garder l'état de l'onglet courant pour le popup (stockage de session), l'identifiant d'installation de l'inventaire de parc (stockage local), et lire le libellé de poste posé par la politique d'entreprise (stockage géré). |
...
- **Divulgations « data usage »** (cases à cocher) :
  - Collecte : « Website content » (performance/erreurs) + « User activity » (mesures de perf). PAS d'authentification, PAS de données financières/santé, aucune coordonnée ni ville. À trancher avant de cocher : l'identifiant de visiteur (pseudonyme aléatoire, `mip.visitor_id`) et le pays estimé côté serveur (fuseau horaire ou en-tête pays du CDN) au regard des catégories « Personally identifiable information » et « Location » de Google — non tranché ici.
  - L'extension envoie aussi à la console un battement d'inventaire (identifiant d'installation tiré au hasard, version, applications alimentées, libellé de poste s'il est posé par la politique d'entreprise), jamais d'URL visitée (`apps/extension/lib/install.ts`).
